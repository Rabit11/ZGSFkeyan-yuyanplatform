...
function allowed(projectId?: number) {
  ...
  return [!admin && code === 'owner' && duty.fill.can && 'fill', !admin && code === 'owner' && duty.submit.can && 'submit', !admin && duty.audit.can && code === 'unitHead' && 'audit', !admin && duty.audit.can && ['hqHead','hqStaff'].includes(code || '') && 'record'].filter(Boolean) as string[]
}
...
function decorate(t: TransformPackage) {
  ...
  return { ...t, confirmedStatus:t.status, confirmedActualDate:t.actualDate, status: t.reportedStatus || t.status, actualDate:t.reportedStatus ? t.reportedActualDate : t.actualDate, workflowStatus: t.workflowStatus || 'DRAFT', revision: t.revision || 0, colorStatus: calcColor(t.planDate,t.status === 'DONE'), allowedActions: allowed(t.projectId), projectName:p?.name, projectNo:p?.projectNo, deliverables:bound(t), deliverableIds:bound(t).map(d=>d.id), itemCount:bound(t).length }
}
function validate(t: TransformPackage, ids: number[]) {
  ...
  if(files.length>20 || files.some(f=>!f.fileName || uploads.get(f.fileUrl)!==t.projectId)) throw Error('佐证材料必须使用平台上传的附件，且最多20份')
  validateIds(t,ids)
}
...
function prepare(t:TransformPackage, previous?:TransformPackage) {
  t.reportedStatus=t.status
  t.reportedActualDate=t.actualDate
  if(t.workflowStatus!=='RECORDED') {
    t.actualDate=previous?.actualDate
    t.status=previous?.status || (t.status==='DONE'?'SIGNED':t.status)
  }
...
export function mockTransform(method:string,path:string,query:any,body:any) {
  if(path==='/files/transform/upload' && method==='POST' && body instanceof FormData) {
    const projectId=Number(body.get('projectId'))
    if(!visibleProjects(currentMockUser()).some(p=>p.id===projectId)||!allowed(projectId).includes('fill')) return fail('无权上传该项目成果材料',403)
    const file=body.get('file') as File
    if(!file?.size) return fail('上传文件不能为空')
    const url=URL.createObjectURL(file);uploads.set(url,projectId)
  ...
  try {
    ...
    if(path==='/transforms/permissions' && method==='GET') {
      const projectId=Number(query.projectId)
      if(!visible.has(projectId))return fail('无权访问该项目',403)
      return ok({allowedActions:allowed(projectId)})
    }
    ...
    t.revision++;prepare(t,existing);Object.assign(existing,t);return ok(true)
