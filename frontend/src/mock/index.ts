...
const DECLARATION_NODE_POST_KEYS: { re: RegExp; keys: string[] }[] = [
  { re: /联系人/, keys: ['contact'] },
...
function isMockDeclarationApprover(row: any, user: any) {
  ...
  const hit = DECLARATION_NODE_POST_KEYS.find((item) => item.re.test(node))
  const labels = (hit?.keys || []).map((key) => posts[key]).filter(Boolean)
  if (labels.length) {
    return canActOnHandlers(labels.map((label) => ({ label: String(label) })), user)
  }
  const identityAllowed = canAuditByIdentity(node, user.identityCode) ||
    (node.includes('分管') && user.identityCode === 'unitStaff')
  ...
  return Boolean(row.applicant && (
    String(row.applicant).startsWith(String(user.realName || '')) ||
    String(row.applicant).startsWith(String(user.username || ''))
  ))
}
...
export async function mockRequest<T = any>(opts: RequestOptions): Promise<Res<T>> {
  ...
  routes.push(
    ...
    m('GET', '/declarations/pending', () => {
      ...
      return ok(DB.declarations.filter((x) =>
        ['SUBMITTED', 'APPROVING'].includes(String(x.status)) &&
        isMockDeclarationApprover(x, user),
      ))
    }),
