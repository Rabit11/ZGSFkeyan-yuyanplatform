...
const canAuditCurrent = computed(() => !!currentFlowNode.value && currentFlowNode.value.nodeType === 'AUDIT' && can.value.audit && !finished.value)
...
function canOperateMaterial(row: any) {
  return !row.locked && !finished.value && (can.value.fill || can.value.edit)
}
    ...
    <a-card :body-style="{ padding: '16px 20px' }">
      <div class="toolbar">
        ...
        <a-space>
          ...
          <a-button type="primary" :disabled="!allPassed || !materialReady || !can.submit || finished" @click="submit">
            提交验收申请
          </a-button>
