...
import { projectApi, deliverableApi, transformApi, fileApi } from '@/api/modules'
...
import { WORKFLOW_TEXT, STATUS_TEXT, WAY_TEXT, ACTION_TEXT, FORM_PARENT, bindingReason, parseArray, validatePackage } from '@/utils/transformPackage'
...
const { can } = useWorkDuty('transform', selectedProject)
const writable = computed(() => form.id ? !!form.allowedActions?.includes('fill') : can.value.fill)
...
const color = (r: Partial<TransformPackage>) => calcColor(r.planDate, r.status === 'DONE' && r.workflowStatus === 'RECORDED')
...
const stats = computed(() => ({ model: rows.value.filter(r => r.transformWay === 'MODEL').length, market: rows.value.filter(r => r.transformWay === 'MARKET').length, done: rows.value.filter(r => r.status === 'DONE' && r.workflowStatus === 'RECORDED').length, risk: rows.value.filter(r => ['RED','YELLOW'].includes(color(r))).length }))
...
const filtered = computed(() => rows.value.filter(r => {
  ...
  return (!kw || [r.name, r.achievementNo, r.projectNo, r.projectName, r.dutyOrg].some(v => String(v || '').toLowerCase().includes(kw)))
    && (!query.way || r.transformWay === query.way) && (!query.status || r.status === query.status)
...
async function openDetail(row: TransformPackage) {
  ...
  try {
    const [res,dvs] = await Promise.all([transformApi.detail(row.id),deliverableApi.list(row.projectId!)])
...
async function changeProject() {
  const seq=++projectSeq; pool.value=[]; form.deliverableIds=[]; detailError.value=''
  const p=selectedProject.value; form.projectNo=p?.projectNo; form.dutyOrg=p?.orgName || p?.leadOrg || ''
  if(!form.projectId) return
  detailLoading.value=true
  try {const res=await deliverableApi.list(form.projectId); if(seq===projectSeq) pool.value=(res.data as ProjDeliverable[]) || []}
...
async function upload(options:any) {
  ...
  try {
    ...
    const res=await fileApi.upload(data,'evidence')
    if(!(res.data as any)?.fileUrl) throw new Error('文件上传未返回有效地址')
    const uploaded = res.data as any
    if (uploaded.objectKey) uploaded.fileUrl = '/api/files/download?objectKey=' + uploaded.objectKey
    evidence.value.push(uploaded); options.onSuccess?.(uploaded)
    message.success('文件已上传，保存成果包后归档')
...
function deadline(r:Partial<TransformPackage>) {
  ...
  if(r.status==='DONE' && r.workflowStatus==='RECORDED') return r.actualDate ? `实际 ${r.actualDate}` : '已完成'
...
<template>
  <div class="page-container transform-page">
    ...
    <section class="ledger-panel">
      ...
      <a-table :columns="columns" :data-source="filtered" row-key="id" :loading="loading" :scroll="{x:1065}" :pagination="{current:query.page,pageSize:query.size,total:filtered.length,showSizeChanger:true,showTotal:(n:number)=>`共 ${n} 项`}" @change="(p:any)=>{query.page=p.current;query.size=p.pageSize}">
        ...
        <template #bodyCell="{column,record}">
          <template v-if="column.key==='name'"><button class="name-link" @click="openDetail(record)">{{ record.name }}</button><div class="record-code">{{ record.achievementNo }}</div><div class="record-project" :title="record.projectName">{{ record.projectNo }} · {{ record.projectName || '—' }}</div></template>
          ...
          <template v-else-if="column.key==='status'"><a-tag :color="record.status==='DONE'?'success':'default'">{{ dict.label('TRANSFORM_STATUS',record.status) }}</a-tag></template>
    ...
    <a-drawer v-model:open="drawerOpen" :title="form.id ? (mode==='edit'?'编辑成果包':'成果包详情') : '新建成果包'" width="min(920px, 94vw)" :mask-closable="false" :closable="!saving&&!uploading" :keyboard="!saving&&!uploading">
      <a-spin :spinning="detailLoading">
        ...
        <template v-else>
          ...
          <a-form layout="vertical" :disabled="mode==='view'||!writable||saving">
          ...
          <template v-if="mode==='view'">
            ...
            <a-button v-if="form.projectId" @click="router.push('/overview/detail/'+form.projectId)">查看所属项目台账 <ArrowRightOutlined/></a-button>
          </template>
        </template>
      </a-spin>
      <template #footer>
