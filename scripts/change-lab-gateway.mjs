// Isolated change rehearsal gateway. No npm dependencies.
...
const server = http.createServer(async (req, res) => {
  if (req.url === '/api' || req.url.startsWith('/api/')) {
    const isLab = req.url.startsWith('/api/change-lab/');
    const port = isLab ? 8081 : 8081;
    const upstreamPath = isLab ? req.url.replace('/api/change-lab/', '/api/') : req.url;
    ...
    upstream.on('error', () => {
      ...
      res.end(JSON.stringify({ code: 502, msg: isLab ? '隔离演练服务暂不可用，请启动测试服务后重试；未切换到原平台数据' : '本地后端暂不可用' }))
    })
...
server.listen(6026, '127.0.0.1', () => console.log('RPM: http://127.0.0.1:6026'))
